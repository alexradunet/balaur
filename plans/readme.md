...
| 052 | Journal + Day: focus editor + retire `/journal`,`/day` (Phase 2) | P2 | M | MED | 050 | — | DONE (journal card focus = the candle at `/focus/journal`; new `day` card — day-of-life tile + `/focus/day?date=…` focus with prev/next; `/journal` + `/day/{date}` retired → 302 /boards, topbar Journal link dropped; all five inbound links re-pointed) |
