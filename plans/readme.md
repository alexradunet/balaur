...
| 125 | Wire `staticcheck` + `govulncheck` into CI & `make lint` (+ `staticcheck.conf` excluding ST1001) | P1 | S–M | LOW | **124** | — | TODO |
| 126 | Modern-stdlib idiom sweep (`modernize -fix ./internal/...`) + `slices` sort/reverse + `time.After` watchdog leak + `Sscanf`→`Atoi` | P2 | S–M | LOW | 124 (soft) | — | TODO |
| 127 | Standardize `gomponents/html` import on the `h` alias (19 feature-card files) | P3 | S–M | LOW | — | — | TODO |
