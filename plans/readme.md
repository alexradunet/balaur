...
| 161 | Wikilinks & backlinks — the `[[double bracket]]` syntax (the ONE link primitive) writing `type="links"` rows into the **`edges`** collection (node↔node `source`/`target` relations; ghost-link stub nodes), `Resolve`/`Resync`/`Backlinks`/`Render` over `edges` (backlinks via expand `edges_via_target`), chip rendering in chat (web-gateway pre-rewrite, `internal/ui/chat` untouched) + a note-card backlinks panel + `main.go` resync hook. Does NOT touch `internal/search` (162 owns it). Block-refs/typed-objects/queries DEFERRED | P2 | M | MED | **160** (HARD — `nodes`/`edges` collections + note card; parser/resolve/render core stands alone) | — | TODO |
