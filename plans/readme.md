...
| 109 | Remove the now-inert theme/mode UI + JS machinery (toggle button, palette pickers, Settings→Appearance tab, storybook switcher, no-flash theme logic, orphaned switcher CSS) | P2 | M | LOW | **108** | — | TODO (not yet written in full — drafted after 108 merges so its excerpts/line-refs match the post-collapse tree. Removes: `basm.js` `basmToggleTheme`/`basmUpdateThemeButtons`/`basmSetPalette`/`basmUpdatePaletteButtons`; `shell.go` no-flash theme/palette class logic; `chatshell.go` `.app-chrome` toggle; `settingsfocus.go` AppearanceSection + the "appearance" tab/section; `storybook.go` footer palette/mode switcher; `stories_navigation.go` theme-toggle in the sidebar fixture; orphaned CSS `.theme-toggle`/`.appearance-themes`/`.appearance-theme-btn`/`.sb-theme-btn`/`.sb-foot-mode`/`.sb-foot-themes`; + the markup tests in `settingsfocus_test.go`/`shell_test.go`/`sidebar_test.go`/`home_test.go`/`cards_test.go` and DESIGN.md topbar-toggle line) |
