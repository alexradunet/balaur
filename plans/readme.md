...
|------|-------|----------|--------|------|------------|-------|--------|
| 143 | Refresh the code tours to the current architecture — re-anchor the 2 tour anchors that point at `web/templates/`+`templates_test.go` (the files 117 deletes) off them, purge boards/htmx/Ollama/`/focus`/`board_compose` prose across 8 tours (07 major-rewrite, 00/06/08 partial, 01/03 prose, 02/04/10 re-anchor, 09 keep) — **prerequisite for 117** | P2 | M | LOW | — | — | TODO (workflow-surveyed: 12 agents, all 22 recommended anchors verified; **hard dep on 117** — tour 6.9→`web/templates/card-memory.html` + 7.6→`templates_test.go` must be re-anchored before 117 deletes those) |
