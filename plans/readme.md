...
Ad-hoc (plan **158**), 2026-06-23, against commit `c57ca72`, from a `plan
<description>` request (owner-reported bug: assistant Markdown shows raw in chat).
Recon-only, no audit. Root cause confirmed by reading the code: `internal/ui/chat/message.go:95`
emits the bubble body via `g.Text(content)` (escaped plain text) for both the
streamed-token morph and the finalized render, so `**bold**`/lists never render. Fix:
convert assistant Markdown → sanitized HTML (goldmark default config + bluemonday
UGCPolicy) and render via `g.Raw`, for balaur turns only (owner turns stay plain;
tool rows out of scope). Single fix point — both render paths funnel through
`messageBody`.

Twenty-third cycle — **PKM / second-brain** (plans **160–163**), 2026-06-23,
against commit `72fd762`, owner-requested feature direction (a LogSeq/Obsidian/
Capacities-style "connective tissue" over Balaur's knowledge). Four plans build
the smallest end-to-end slice of a personal knowledge system on a **greenfield
`nodes` + `edges` spine** (pb_data disposable, no data migration — the rewritten
baseline IS the final schema), each a self-contained handoff for a weak executor:
**160** rewrites the consolidated baseline migration to add the `nodes` collection
(notes/typed-objects/journal born `status=active`; memory/skill born `proposed`,
consent-gated) and the `edges` collection (node↔node, relation fields `source`/
`target`, cascade-delete, unique (source,target,type)), removes the standalone
`memories`/`skills` collections, splits journal out of `entries`, and adds the
`internal/nodes` domain package + node tools + a `note` card; **161** adds the
`[[double bracket]]` wikilink syntax — the one link primitive — writing
`type="links"` rows into the **`edges`** collection (ghost-link stubs, backlinks
via back-relation expand `edges_via_target`), with chip rendering in chat and a
note-card backlinks panel; **162** owns ALL `internal/search` changes, widening
the FTS5 sidecar from `memories_fts` into one unified
`knowledge_fts(id UNINDEXED, kind UNINDEXED, title, content, extra)` table over
**`status=active` nodes** of every type, and adds a `search` agent tool (`recall`
stays memories-equivalent); **163** ships the "see the network" surface — a
related-items list and a server-rendered 1-hop SVG graph card that **READS the
`edges` collection** through 161's `internal/nodes` Backlinks/Outbound helpers
(status=active only; force-directed/whole-vault DEFERRED). Both search and graph
filter to `status=active` so agent-proposed-but-unapproved nodes never surface.
Block refs (`((…))`), Capacities typed objects, LogSeq queries, and tasks-as-nodes
(note↔task cross-layer links) are explicitly DEFERRED across all four (the
double-bracket wikilink is the only link syntax; edges are node↔node only).
**Only 160 declares the `nodes`/`edges` migration; 161/162/163 read/extend it and
never redeclare. 162 owns all `internal/search`/FTS changes; 163 reads similar
nodes only through 162's helper and never touches the index.** **Dependency graph:
160 → {161, 162, 163}; 161 → 163; 162 soft → 163.** 161 hard-depends on 160;
162 hard-depends on 160 (it codes defensively if 160 has not landed but the
`nodes` collection must exist); 163 hard-depends on both 160 (focus node + note
card + the `GET /ui/show/{type}` dispatcher) and 161 (the `edges` rows it reads),
and softly benefits from 162 (FTS-similar items, which degrade gracefully when the
index is absent). **Recommended execution order: 160 → 161 → 162 → 163.**

## Execution order & status

| Plan | Title | Priority | Effort | Risk | Depends on | Issue | Status |
|------|-------|----------|--------|------|------------|-------|--------|
| 160 | Greenfield knowledge spine — rewrite the consolidated baseline migration to `nodes` + `edges` (edges.source/target relation fields, cascade-delete, unique (source,target,type) + (target) index). REMOVE `memories`/`skills` collections; SPLIT journal out of `entries` (journal → `type=journal` nodes; entries stays life-log measures/lines). Carry the consent lifecycle onto node status (note/journal/typed-objects born active; memory/skill born proposed). New `internal/nodes` domain pkg (CRUD + edge/backlink/neighborhood helpers, status=active traversal filter). Port `internal/knowledge`/`internal/life` journal/`internal/search` hooks/cards/CLI/seed/self onto nodes; add `node_write`/`node_list`/`node_get`/`node_drop` tools + a `note` card so `/ui/show/note?id=…` resolves. Greenfield (pb_data disposable, no data migration). 161/162/163 read/extend nodes+edges, never redeclare | P1 | L | HIGH | — | — | DONE — APPROVED by advisor; executed + reviewed, **merged to main `6ab038a`** (`merge: 160`, not pushed). Dispatched opus executor → worktree `agent-a178dc70d99d1b636`, branch `improve/160-nodes-edges-spine`, commit `067797a` (33 files, +1656/−513). Advisor re-ran ALL gates in the worktree (did NOT trust the report — its COMPLETE claim coincided with IDE compiler-error diagnostics for `nodeEdit`/`noteCmd`/`registerNote`, ALL of which proved stale mid-edit snapshots: each symbol is defined+used in the committed tree): `CGO_ENABLED=0 go build ./...` exit 0; `go vet` exit 0; `gofmt -l` clean; env-scrubbed `go test ./...` all pass (incl. new `internal/nodes`); `make lint` (staticcheck+govulncheck+test) exit 0. Done-criteria greps: nodes+edges present, memories/skills 0, 3 node hooks / 0 memory hooks, search gate `type != "memory"` + `body` reads present. Read `nodes.go` (consent filter centralized in `activeByIDs`; `AddEdge` idempotent; audit-after-write incl. rejected transitions) + `nodes_test.go` (`TestTraversalExcludesNonActive` genuinely asserts proposed-node exclusion from Backlinks/Outbound/Neighborhood; `TestDropCascadesEdges` proves PB cascade). Relation names landed as `source`/`target` + `CascadeDelete:true` (the 161/163 contract held). Scope = 33 files, all in-scope or 3 documented minimal deviations (web.go +1 `/ui/node/{id}/edit` route; dayfocus.go +5 journal-node render-bug fix from the port; story.go +1 story registration). Browser /verify performed by executor (note/memory/skill/day cards render from nodes). **Deferred (non-blocking):** note-card body renders as escaped `g.Text` not markdown (layering-safe, like the skill card); no `note` palette item (YAGNI — `/ui/show/note` needs an id; notes-list is 161/163); 1 trivial `modernize` newexpr hint (non-gating). **Landed `--no-ff` into main `6ab038a`:** the branch (off `72fd762`) 3-way-merged against a parallel `fixes` commit (`0f09a56`) — only conflicts were generated `graphify-out/*` (resolved by re-running `graphify update` on the merged tree + minify) and `story.go` (git auto-merged both story registrations). Merged-tree `CGO_ENABLED=0 go build` + `go vet` + full env-scrubbed `go test ./...` all green; worktree + branch deleted. Not pushed. |
| 161 | Wikilinks & backlinks — the `[[double bracket]]` syntax (the ONE link primitive) writing `type="links"` rows into the **`edges`** collection (node↔node `source`/`target` relations; ghost-link stub nodes), `Resolve`/`Resync`/`Backlinks`/`Render` over `edges` (backlinks via expand `edges_via_target`), clickable `[[ ]]` chips via an extended `internal/ui/chat` markdown renderer (`RenderMarkdownLinked`) — in BOTH the chat bubble AND the note-card body — plus a note-card "Linked from" backlinks panel + a `main.go` resync hook. Reuses 160's `internal/nodes` `AddEdge`/`Backlinks`/`Outbound` (no redeclaration). Does NOT touch `internal/search` (162 owns it). Block-refs/typed-objects/queries DEFERRED | P2 | M | MED | **160** (HARD) | — | DONE — APPROVED + **merged to main `0c85d0e`** (`merge: 161`, --no-ff; graphify regenerated into the merge commit), not pushed. Reconciled to post-160 first (`9fefeea`); 2 advisor-decided design conflicts: clickable note-body `[[links]]` via the shared chat renderer (reverses 160's keep-markdown-out-of-cards deferral), and a `LinkedFrom([]BacklinkView)` view-model. Dispatched opus executor → worktree, commit `d1f6c32` (9 files, +609/−29). Advisor re-ran ALL gates in the worktree AND on merged main (did NOT trust the report): `CGO_ENABLED=0 go build`/`go vet`/`gofmt -l`/env-scrubbed `go test ./...`/`git diff --check` all green. Read `links.go` (consent filter in `resolveOrCreateStub` — resolves only `status='active'`, else creates an active stub; `SyncLinks` full-replace via 160's idempotent `AddEdge`, no redeclaration, no raw edge save) + `links_test.go` (`TestProposedNodesNeverResolve` genuinely asserts a proposed node is never resolved — an active stub is made instead; + idempotency/rewrite/self-edge/stub) + `markdown.go` (`RenderMarkdownLinked` injects chips POST-sanitize with escaped display+id — XSS-safe; `renderMarkdown` byte-identical) + `markdown_test.go` (the 2 load-bearing empirical checks + the `[[<script>]]` XSS case) + `note.go` (resolution 1: `buildNote` pre-renders `BodyNode` via a read-only active-title resolver; resolution 2: `[]BacklinkView`). Scope = 9 in-scope files; executor reverted graphify (advisor regenerated on merge); `internal/search` + `migrations/` untouched. Executor did live E2E browser verification (node create → active stub + 1 edge + `graph.stub` audit; idempotent re-save; chips render in storybook). staticcheck absent in env (vet clean). Worktree + branch deleted. Not pushed. |
| 162 | Unified full-text search — **owns ALL `internal/search`/FTS changes**: widen `memories_fts` → one `knowledge_fts(id UNINDEXED, kind UNINDEXED, title, content, extra)` table over **`status=active` nodes** of every type (kind = node `type`); kind-aware `Rebuild`/`Upsert`/`QueryKind`; multi-kind write hooks; new `search` agent tool (`recall` stays memories-equivalent). status=active consent filter so proposed nodes never surface. Index DB disposable — no migration. Codes defensively if 160 not-yet-landed | P2 | M | MED | **160** (HARD) | — | DONE — APPROVED + **merged to main `b5b200a`** (`merge: 162`, --no-ff), not pushed. Reconciled to post-161 first (`e916524`); 5 advisor-decided conflicts: category kept (folded into `extra`); recall stays memory-scoped via `QueryKind` (`SearchActive` unchanged but for that one call); new `SearchAllActive` for cross-type; inverted gate test deleted; flat `balaur search`. Executor STOPPED honestly on a stale worktree base (pre-160) → re-based to `e916524` via SendMessage, then COMPLETE (commits `4ef7bba`+`e312841`). Advisor re-ran ALL gates on the branch AND merged main: build/vet/`gofmt -l`/env-scrubbed `go test ./...`/`diff --check` all green; staticcheck+govulncheck clean. Read `index.go` (`knowledge_fts(id,kind,title,content,extra)`; Rebuild drops `type='memory'` keeps `status='active'`; `nodeExtra` folds when_to_use+category for memory parity; Upsert consent-skip; `matchExpr`/`scanIDs` shared; `Query` all-kinds + `QueryKind` memory) + `knowledge.go` (`SearchActive`→`QueryKind(Memory)`, hydration/sort/fallback UNCHANGED; new `SearchAllActive` raw cross-type, consent-filtered) + `index_test.go` (`TestSearchConsentFilter` proposed-never-indexed BOTH paths; `TestRebuildMultiKind` all types index + `QueryKind` narrows). bm25 + injection-safe quoting intact. Scope = 11 files; `migrations/` + `internal/nodes/` untouched, 161's `registerGraphLinks` preserved. Runtime smoke: `balaur search` cross-type, `balaur memory recall` memory-only. Worktree+branch cleaned. Not pushed. |
| 163 | See the network — a related-items list (backlinks + outbound + FTS-similar) + a server-rendered 1-hop SVG `graph` card (radial layout, no JS physics), both new card types in a new `graphcards` feature pkg. **READS only** — the `edges` collection via 161's `internal/nodes` Backlinks/Outbound helpers (`edges_via_target`/`edges_via_source`) + `nodes` (160); `status=active` only; FTS-similar read through 162's helper, degrades gracefully without 162. Does NOT touch `internal/search` or the migration. 1-hop only; force-directed/whole-vault DEFERRED | P2 | M | MED | **161** + **160** (HARD); **162** soft | — | DONE — APPROVED + **merged to main `2dbdcb2`** (`merge: 163`, --no-ff), not pushed. **Completes the PKM chain 160→163.** Reconciled to post-162 first (`4fa858f`); the 5 flagged items were all factual corrections already applied (no advisor fork) + confirmed the one-generic-node-card design (`/ui/show/note?id=` serves any node type, type shown as a badge). Dispatched opus executor with a rebase-to-HEAD **step 0** (the worktree-base fix) → COMPLETE, commit `6d78b28`. Advisor re-ran ALL gates on the branch AND merged main: build/vet/`gofmt -l`/env-scrubbed `go test ./...`/`diff --check` green; `staticcheck ./internal/feature/graphcards/...` exit 0 (the "unused buildRelated/buildGraph" diagnostic was a stale gopls false-positive — both are called from `graphcards.go:21,24`). Read `related.go` (Backlinks∪Outbound∪FTS-similar via `SearchAllActive`, de-duped, focus excluded, active-only) + `graphcards_test.go` (`TestRelatedComputation` asserts a proposed neighbor is absent; `TestGraphCardRendersSVG` asserts a proposed node never reaches the SVG + empty-neighborhood case). READ-ONLY scope held: 12 files; dispatcher/`internal/search`/`internal/nodes`/`internal/knowledge`/`migrations` all untouched. Deviations: internal test package to drive the unexported builders (avoids exported-for-test dead code); `cards_test.go` count-sync traceable to the 2 spec additions. No palette item (parameterless launcher needs a runtime id → 400). Browser-verified (storybook related + graph cards render). Worktree+branch cleaned. Not pushed. |
| 159 | Make tool-calling reliable & observable: (1) Debug trace of tools-offered/tool-calls-returned in the agent loop (inject `app.Logger()`); (2) moderate `temperature` (0.3 const) in BOTH `llm.OpenAIClient` and `kronk.Client` (neither sets sampling today → providers default ~0.7); (3) `content: null` (not `""`) on assistant tool-call messages for strict OpenAI-compat providers (Mistral); (4) `delta`→`message` fallback in the OpenAI stream parser (kronk bridge already has it). `tool_choice`/system-prompt OUT of scope | P1 | M | LOW–MED | — | — | DONE — APPROVED + merged to main (`merge: 159`). Generated by improve skill at `2b03f17`. Dispatched sonnet executor → worktree, commit `28f0578`, scope clean (7 in-scope files, +200/−22, zero out-of-scope). Advisor re-ran ALL done criteria in the worktree: CGO-free build / `go vet` / `go test ./...` (37 pkgs) / staticcheck / gofmt / `git diff --check` all green; greps confirm `temperature` in both clients, `Content *string` (null-able), `ev.Choices[0].Message` fallback, `Logger` field + `app.Logger()` injection. Read full diff — matches plan intent: dual `agentTemperature=0.3` consts (cross-ref comment, no cross-pkg dep), `toWire` nulls content ONLY on assistant+tool_calls+empty, `streamPayload` factored for the delta→message fallback (mirrors kronk bridge), nil-safe loop logger. 4 new tests audited & meaningful (temperature-in-body, null-vs-string content, message-fallback assembles the call, loop logs `tool_calls`). No STOP conditions. Note: runtime needs a restart to load it. |
| 158 | Render Markdown in chat message bubbles — assistant turns only. New `internal/ui/chat/markdown.go` (goldmark + bluemonday `UGCPolicy`, `g.Raw` fallback to `g.Text` on error); thread `markdown bool` through `messageBody`; `.cmsg-md` prose CSS (pre-wrap off); storybook variant. `chatstream.go` UNCHANGED (funnels through `chat.Message`/`MessageBody`). User turns + tool rows out of scope | P1 | S–M | LOW-MED | — | — | DONE — APPROVED, executor commit `ce8f428`. Dispatched sonnet executor → worktree, scope clean (7 files, +98/−7), `chatstream.go` untouched (confirmed `git diff --stat`). Advisor re-ran all done criteria in the worktree: CGO-free build/`go vet ./...`/`go test ./...` (all pkgs incl. chat+storybook) green; `git diff --check` clean. Diff matches plan exactly — `messageBody` gains `markdown bool`, `!user` wired at the call site, exported `MessageBody`→true (streaming is always assistant), `renderMarkdown` = goldmark-default (no `WithUnsafe`) → bluemonday `UGCPolicy` → `g.Raw`, falling back to `g.Text` on error. Tests meaningful (asserts `<strong>`/`<ul>`/`<li>` present AND `**bold**` absent; `TestMessageUserDefaultName` unchanged — user turns stay plain). One REVISE round: executor first left goldmark+bluemonday as `// indirect` in go.mod (wrong — `internal/ui/chat` is in the root module); fixed via `go mod tidy` + `--amend`, now direct requires, tidy is a no-op. |
| 154 | Remove redundant `llm_providers.local` bool — derive locality from `kind` (sort + `findOrCreateLLMProvider` + `configFrom` + 3 audit maps + `self.go` + test) | P2 | S | LOW | — | — | DONE — APPROVED + merged to main `e4f6e2d` (`merge: 154`). Dispatched sonnet executor → worktree, commit `902a416`, scope clean (3 files, +13/−21). Advisor re-ran done criteria in the worktree + combined main: CGO-free build/vet/`go test ./...` (36 pkgs)/staticcheck all green; `.Local`/`GetBool("local")`/`Set("local")` greps empty. Diff behavior-preserving — the sort first-`if` dropped (the `Kind=="local"` comparison already gives identical order), 3 audit maps drop `"local"` (kind conveys it), `findOrCreateLLMProvider` `local` param + change-detect + write removed, `configFrom` read removed, 3 test assertions simplified to Kind-only. `local` column stays in schema until 156. |
| 155 | Remove redundant `skills.enabled` bool — derive from `status` (knowledge lifecycle+queries, skill cards, CLI export). Scope trap: `llm_models`/`llm_providers.enabled` stays | P2 | S–M | MED | — | — | DONE — APPROVED + merged to main `9a1d2e1` (`merge: 155`). Dispatched sonnet executor → worktree, commit `160e617`, scope clean (5 files, +8/−12). Advisor verified on combined main: build/vet/`go test ./...`/staticcheck green; `enabled = true`/`Set("enabled")`/`GetBool("enabled")` gone from knowledge/cards/cli, `llm_*.enabled` in `store` untouched (scope trap held). All 4 reads derive `r.GetString("status") == knowledge.StatusActive`; queries simplified to `status='active'`; documented in-scope test fix (`knowledge_test.go:163` asserts a proposed skill is not active — equivalent invariant). `enabled` column stays in schema until 156. |
| 156 | Collapse the 21 churned migrations into one clean schema baseline — rewrite `1749600000_init.go` (14 collections), delete 20 migrations + 6 coupled tests, add `schema_test.go`. Folds: drop dead `summary`/`tags`, drop redundant indexes, add `idx_memories_status_importance` + `importance` 1–5, `base_url` Max 2048, remove dead `cli/audit.go` head read. Keeps `entries.value` (seed marker). **Requires fresh `pb_data/`** | P1 | M | LOW | 154, 155 | — | DONE — APPROVED + merged to main `23e5a28` (`merge: 156`), pushed. Dispatched opus executor → worktree, commit `e3966c8` (+502/−2118 across 33 files). Advisor re-ran on combined main: CGO-free build/vet/`go test ./...` (exit 0, 36 pkgs)/staticcheck/gofmt/`diff --check` all green; `migrations/` now = `1749600000_init.go` + `schema_test.go` + `timestamp_uniqueness_test.go`. Read `InitCollections` in full — matches spec exactly (14 collections in dep order, owner rules, `api_key` SetHidden, `importance` Min/Max 1–5 compiled cleanly — no fallback, `entries.value` kept, seed `soul_avatar=male`). `schema_test.go` is a real contract test (positive + negative: `summary`/`tags`/`enabled`/`local`/`head` absent, `idx_messages_conversation`/`idx_tasks_status`/`idx_audit_created` absent, `boards`/`grants` absent) and passes against a fresh `storetest` DB. **Two repo-mandated documented deviations approved on merit:** `.tours/06` anchor 6.10 retargeted off the deleted `1749700000_knowledge.go` → `1749600000_init.go:86` (the `memories` block; `tours_test` requires it, honest rewrite); `graphify update` refreshed `graph.json` (minified) + `GRAPH_REPORT.md` (CLAUDE.md mandates it). Requires fresh `pb_data/` at runtime (existing dev boxes drop the dir). |
| 157 | Rename `heads.tools` → `capability_groups` (disambiguate from `extensions.tools`) — column + 2 accessors only; form field name unchanged | P3 | S | LOW | 156 | — | DONE — APPROVED + merged to main (`merge: 157`), pushed. Dispatched sonnet executor → worktree, commit `06ca91d`, scope clean (3 files: `init.go` + `heads.go` + `schema_test.go`, plus the CLAUDE.md-mandated graphify refresh). Advisor verified on combined main: build/vet/`go test ./...` (exit 0, 36 pkgs)/staticcheck green. Two-occurrence trap handled correctly — `init.go:45` = `capability_groups` (heads), `init.go:213` = `tools` (extensions, untouched); `heads.go` has 0 `"tools"` / 2 `capability_groups` (Set + GetString); scope-trap `tools` in `ext`/`self`/`cli` all preserved. Form field name `tools` left as-is (transport key, not the DB column); `schema_test` asserts heads has `capability_groups`, not `tools`. |
...
| 150 | Extract one shared `internal/uitest.Render` test helper — 8 byte-equivalent `render(t,n)` copies converted to one-line delegates (KISS slice: keep each helper's name/signature, touch ZERO call sites); test-only, per-file import math tabulated | P2 | M | LOW | — | — | DONE — APPROVED + merged to main `0ce6403` (`merge: 150`), pushed. Advisor-reviewed: new `internal/uitest.Render` (storetest-style), 8 byte-equivalent helpers → one-line delegates with zero call-site churn, import math correct (`bytes`/`strings` dropped where now-unused), `TestNoWebImports` preserved; +42/−52; suite green on combined tree. |
| 151 | Collapse the three cron registrars into one `scheduleJob` helper in `main.go` — owns the per-job mutex/TryLock/Active-check/`MustAdd`/goroutine; `tolerateNoModel` flag encodes recap(needs model) vs nudge/briefing(tolerate nil) | P2 | S | LOW | — | — | DONE — APPROVED + merged to main `1e7844a` (`merge: 151`), pushed. Advisor-reviewed: `scheduleJob` owns per-job (closure-scoped) mutex/TryLock/`Active`/`MustAdd`/catch-up goroutine; `tolerateNoModel` reproduces recap-skip vs nudge/briefing-nil; recap's `conversation.Master` precondition kept in its body; main.go stays thin; +33/−41; suite green on combined tree. |
| 152 | Consolidate web SSE render/patch boilerplate — `patchOuter`(node)+`patchOuterHTML`(string) helpers over the existing `renderNodeHTML` collapse the "build→render→PatchElements(outer)" tail across 8 `internal/web` files; error-checked renders kept out of scope (1:1, behavior-preserving) | P2 | M | LOW | — | — | DONE — APPROVED + merged to main `5aeda4a` (`merge: 152`), pushed. Advisor-reviewed: `patchOuter`(node)/`patchOuterHTML`(string) over the existing `renderNodeHTML`, 19 call sites collapsed across 8 `internal/web` files, `chatStream.renderNode` delegates, excluded sites (patchChatbar/inner-mode) untouched (`WithModeOuter` grep confirms); suite green on combined tree. |
| 153 | Clear the cycle-15 trivial simplification sweep — delete dead `internal/web/settings.go` (empty stub) + write-only-dead web avatar plumbing (`AvatarOption`/`buildAvatarOptions`/`AvatarOptions` field, live copy is `settingscards.ProfileView`); `itoa`→`strconv.Itoa` (questsfocus); collapse `clip`/`clipDay` dups onto `ui.Clip`. Re-vetted: EXCLUDES `turn.ClientFor`/`heads.Builtins`/`OpenAIClient.HTTP`/`enumContains` (don't survive vetting). Cosmetic idiom swaps (clamps→builtins, sort→slices, min/max loops) listed as optional/lateral | P3 | S | LOW | — | — | DONE — APPROVED + merged to main (`merge: 153`, `--no-ff`), pushed. Dispatched sonnet executor → worktree `agent-a6162df7f050e4cf0`, commit `1c52622`, 5 files (−92/+14). Advisor re-ran ALL done criteria in the worktree: CGO-free build/vet/`go test ./...` (36 pkg ok, 0 FAIL)/staticcheck (0 output)/`gofmt -l internal/`/`git diff --check` all green. Scope = exactly the 5 in-scope files, no test files, nothing out of scope. Diff read: behavior-preserving — `ui.Clip` byte-identical to the deleted `clip`/`clipDay`; `strconv.Itoa` matches `itoa` for all ints; dead `buildAvatarOptions`/`AvatarOption`/field/assignment removed (live copy `settingscards.ProfileView` confirmed intact, 13 matches); `lifelogfocus.go` already imported `ui` (no import churn), `journalcards/dayfocus.go` gained the `ui` import. **Follow-up surfaced:** the comment at `settingscards/settingsfocus.go:57` ("Mirrors AvatarOption in internal/web/models.go") now references a deleted symbol — minor stale-comment, out of scope, candidate for a future docs pass. |
...
- **The ~20 trivial do-now wins** — **now packaged as plan 153** (sixteenth cycle,
  2026-06-22, re-vetted at `61e79d7`). Re-vetting confirmed the deletion-flavored wins
  (dead `web/settings.go`, write-only-dead web avatar plumbing, `itoa`→`strconv.Itoa`,
  `clip`/`clipDay`→`ui.Clip`) and **dropped** several originally-listed items that don't
  survive: `turn.ClientFor` (a later cycle rejected it — differs in `embedModel`, backs a
  test), `heads.Builtins()` (backs `heads_test.go`), `OpenAIClient.HTTP` (deliberate read
  seam), and `enumContains` (already delegates to `slices.Contains` after plan 126). The
  remaining cosmetic idiom swaps (clamps→`max/min` builtins, `sort.*`→`slices`, manual
  min/max loops) are LOW-value/lateral and live in 153's optional appendix. Original
  catalog preserved below for the record:
  reinvented-stdlib swaps — `itoa()`→`strconv.Itoa`
