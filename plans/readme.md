...
| 086 | One-click "Get our official model" — in-app GGUF download + verify + auto-activate | P1 | M–L | MED | — (087 resolves its runtime-missing Alert) | — | DONE (reviewed; `improve/086-official-model-download` in worktree `agent-ad08b56afdaf0727c`; build/vet/`go test ./...`/gofmt all green, scope clean. Caught+fixed a real bug — in-flight guard `Store().Set(key,nil)`→`Remove` (GetOk is presence-based, else download-once-per-boot). NOT MERGED. 2 pre-merge actions: (a) commit the 2 uncommitted files (models.go bug fix + README note); (b) fill the real sha256+size in officialmodel.go & drop the `!= "REPLACE_WITH_REAL_SHA256"` skip in modelget.go:216) |
