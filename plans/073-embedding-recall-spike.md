...
## Spike findings (filled by executor)

### Final `RerankByEmbedding` signature as implemented

```go
func RerankByEmbedding(ctx context.Context, client llm.Client, query string, candidates []*core.Record) []*core.Record
```

No deviation from Step 2. The function is standalone in `internal/knowledge/rerank.go`; it takes candidates and a client explicitly rather than widening `SearchActive`. Returns the slice directly (no error) — failures silently restore input order.

### Candidate text choice

Each candidate is embedded as `title + ": " + content` (when content is non-empty and differs from title), else just `title`. This mirrors `writeMemoryLine` in `context.go:74-84` and uses the same fields the FTS5 index indexes (`title`, `content`). It is the simplest correct choice that keeps parity with the lexical stage's field selection.

### K decision

The spike reranks whatever `SearchActive` returned — the caller's existing `recallLimit = 6`. No separate larger K is introduced. **Open question for follow-up**: widening the lexical net (e.g. fetch top-24 from FTS, rerank to top-6) could materially improve recall quality at the cost of 4× more embeddings per query. Worth measuring after the basic wiring lands; do not guess.

### Caching — the #1 follow-up

The spike recomputes all candidate embeddings per query (one `Embed` call for `[query] + candidates`). This is cheap to implement and correct, but its per-turn cost scales with `recallLimit × embed_latency`. Three options for the real feature to choose between:

- **(a) Sidecar vector column** alongside the FTS5 index in `internal/search`: embed on `Upsert`/`Rebuild`, persist the vector (e.g. as a JSON blob or a `BLOB` column), do cosine in Go after FTS retrieval. Eliminates the per-query embed cost entirely; recomputes only on memory change. **Recommended** — it amortizes cost at write time, keeps the vector co-located with the index that uses it, and fits the existing `internal/search` `Upsert`/`Rebuild` seam naturally.
- **(b) Recompute top-K per query** (what the spike does). Cheapest to build; expensive per turn. Acceptable only if embed latency under `embeddinggemma` locally is sub-10 ms; must measure before shipping.
- **(c) In-process LRU** keyed on record ID + content hash. More moving parts than (a) with no clear advantage; skip.

### Wiring into the live path — the #2 follow-up

Neither `SearchActive` (`internal/knowledge/knowledge.go`) nor `BuildContext` (`internal/knowledge/context.go`) carry an `llm.Client`. Threading one in is a 4-caller ripple:

1. `internal/knowledge/context.go:45` — `BuildContext` (the hot path)
2. `internal/cli/knowledge.go:156` — CLI search subcommand
3. `internal/tools/knowledge.go:122` — the model's `recall` tool
4. `internal/knowledge/knowledge.go:233` — `SearchActive` itself (if the rerank is embedded there rather than as a post-step in callers)

The cleanest approach for the follow-up is to keep `SearchActive` unchanged and instead add an optional post-step in `BuildContext` only (the per-turn caller), threading the local embed client down from `internal/turn`. The CLI and tool callers can remain unchanged — they do not need reranking on first ship.

### Local-vs-remote decision (hard requirement for follow-up)

`RerankByEmbedding` must always be passed the LOCAL Ollama embed client (configured via `BALAUR_EMBED_MODEL` / `embeddinggemma`), never the active remote chat client. Memory content must not flow to a remote provider unless the owner explicitly opts in through a dedicated future setting. This is a hard requirement from the README/AGENTS safety rule ("keep secrets out of content sent to remote providers"). The spike's test uses the `llmtest` fake and makes no network calls; the constraint is design-level, not yet code-enforced, because `RerankByEmbedding` is unwired from the live path.

### Measurement note

This spike has NOT measured real embed latency or recall quality against a live model. All tests use the `llmtest` scripted fake. **The follow-up must measure before shipping:**

1. Per-turn added latency with `embeddinggemma` locally (target: imperceptible at conversational pace; if >50 ms, caching option (a) is mandatory).
2. Whether reranking actually improves recall on a small labeled set of owner messages — the rerank is only worth wiring if it demonstrably reorders at least some queries toward more relevant memories. Measurement-first before promoting `RerankByEmbedding` into `BuildContext`.
