...
> `git diff --stat a16cb9d..HEAD -- internal/ui/shell/shell.go internal/ui/shell/shell_test.go internal/web/web.go internal/web/focus.go internal/web/boards.go internal/web/focus_test.go internal/web/boards_test.go internal/web/handlers_test.go internal/tools/ui.go internal/tools/ui_test.go internal/turn/tools.go internal/feature/storybook/stories_navigation.go internal/feature/storybook/story.go migrations/1750740000_boards.go web/templates/focus.html web/templates/boards.html web/templates/layout.html DESIGN.md internal/self/knowledge.md`
> If any changed since `a16cb9d`, compare the "Current state" excerpts below to the live code; on mismatch NOT explained by the reconciliation note below, STOP and report which excerpt drifted.
>
> **RECONCILED FOR POST-088 (2026-06-17, anchor `a16cb9d`) — READ FIRST.** Plan 088 has **LANDED and merged to main** (`82503be`; HEAD `a16cb9d`). Its changes to files this plan also touches are EXPECTED — reconcile against the live tree, do NOT treat them as STOP-worthy drift. Confirmed post-088 facts:
> - `internal/ui/shell/chatshell.go` exists (`func ChatShell`); `GET /ui/show/{type}` is registered (`web.go:238`). The dependency gate below is SATISFIED.
> - **`home.go` NO LONGER calls `shell.Page`** — it uses `shell.ChatShell` now. So §2's "three callers" is stale: `shell.Page`'s live callers are now `internal/web/web.go:267` (`renderPageError`) and `internal/web/focus.go:178` (deleted by this plan). **DECISION (overrides Step 1's branch guessing): you DELETE `focus.go`, so after that `shell.Page`'s ONLY caller is `renderPageError`. KEEP `shell.Page` + `PageProps` but EDIT `Page` to drop the `Topbar(p.Active)` and `topnavDrawer(p.Active)` calls (it becomes skip-link + `.with-sidebar > #main(Body)` + `#dock`). Do NOT delete `shell.Page` — `renderPageError` needs a Body/`#main` shell that `ChatShell` (sidebar+dock only, no Body slot) does not provide. The `Active`/`HTMLClass` fields may stay unused or be trimmed if `Page` no longer reads them.** Then delete `Topbar`/`topbarLinks`/`navLink`/`topnavDrawer`/`focusActiveKey`.
> - The `web.go` route block shifted +2 (088 inserted `/ui/show` at `:238`): `/focus/{type}` is now `:239`; the eight `/boards*` routes are `:241-248`. Grep, don't trust the old line numbers.
> - **Storybook now has BOTH `sidebarStory()` (088, `stories_navigation.go:110`, registered `story.go:100`) AND `topbarStory()` (`stories_navigation.go:178`, registered `story.go:101`). REMOVE ONLY `topbarStory` + its registration; KEEP `sidebarStory` (it is 088's, in scope for neither removal).**
> - `internal/self/knowledge.md` was already rewritten by 088 to the HYBRID interim state (it still describes `/focus/*` + `/boards` as live, ~8 mentions). Your Step 8 FINISHES the rewrite: remove the `/focus`/`/boards` mentions so it describes the FINAL single-page IA. Read the CURRENT (post-088) `knowledge.md`, not the pre-088 excerpts.
> - `internal/web/assets/static/basm.css` got 088's `.app-shell`/`#dock.app-dock`/`.sb-nav-icon` rules appended — LEAVE them (CSS hygiene is plan 091). `handlers_test.go`/`home_test.go` had their home assertions migrated by 088 — your only `handlers_test.go` change is the `/boards`→`/` retired-route wording.
> - Migration `1750850000` is still FREE (latest on disk is `1750840000`).
>
> **Dependency gate**: SATISFIED (088 merged — see above). If for some reason `grep -rn "func ChatShell" internal/ui/shell/` or `grep -n "/ui/show/{type}" internal/web/web.go` comes back EMPTY, STOP (088 was reverted).
