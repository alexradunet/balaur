...
`entries` life log, every mutation audited.

**Roadmap — do not state as shipped:** task nudger + morning briefing +
/tasks page · life tracking (weight, workouts, achievements, /life) · day
pages with journaling · Johnny Decimal Markdown vault mirror (one-way
export + git) · FTS5/embedding recall · encrypted export · multi-human
accounts · channel adapters (Signal/WhatsApp/web) · CLI client.
