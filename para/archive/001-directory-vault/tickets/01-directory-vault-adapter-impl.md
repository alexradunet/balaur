...
- [x] Created `storage/directory-vault.js` per plan §1, literally:
  ...
  - `snapshot`: `{ format: "orbit-vault-snapshot", revision, files: [{ path, mediaType, text }] }` sorted by path. `restore`: validate every path + reject in-snapshot case-fold collisions up front, remove the whole current file tree, reset `_journal`/`_revision`, write each file via `this.write(..., { expectedHash: null })`, return `{ revision, count }`.
