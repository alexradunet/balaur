...
- `sw.js` maintains the versioned `orbit-shell-v13` cache.
...
The install handler precaches the application shell in `orbit-shell-v13`:
...
3. Confirm `orbit-shell-v13` contains every `APP_SHELL` URL, no failed asset, no database Wasm, and no arbitrary vault file.
...
await caches.keys()
(await caches.open("orbit-shell-v13")).keys()
