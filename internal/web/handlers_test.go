...
func TestChatHandler(t *testing.T) {
	...
	scenarios := []tests.ApiScenario{
		{
			// The chat stream is Datastar SSE: the owner row is echoed and the
			// model text is morphed into the assistant bubble body. Both appear
			// in the datastar-patch-elements events.
			Name:            "chat streams over SSE",
			Method:          "POST",
			URL:             "/ui/chat",
			Body:            strings.NewReader(`{"message":"hello"}`),
			Headers:         map[string]string{"Content-Type": "application/json"},
			ExpectedStatus:  200,
			ExpectedContent: []string{"event: datastar-patch-elements", "Hello from the fake model", "msg msg-user"},
		},
		{
