...
export default function (pi: ExtensionAPI) {
  ...
  async function status(params: any, signal?: AbortSignal) { const handle = requireHandle(params.handle); try { const agent = await pinned(handle, signal); handle.status = agent.agent_status; save(handle); } catch { if (handle.status !== "replaced") { handle.status = "missing"; save(handle); } } return { content: [{ type: "text", text: bounded(`${handle.handleId}: ${handle.status}\nagent=${handle.agentName}\npane=${handle.paneId}\nsession=${handle.sessionKind}:${handle.sessionValue}`) }], details: details({ handle: handle.handleId }) }; }
