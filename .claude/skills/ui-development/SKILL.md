---
name: ui-development
description: Use when building or changing Balaur's web UI — creating or modifying gomponents components (atoms, chat organisms, domain cards, the page shell), wiring Datastar interactions, or adding/updating storybook stories. Covers the Hearthwood design system, the component layers, the storybook-as-source-of-truth workflow, the CSS/token conventions, and the Datastar @post/SSE contract.
---
...
| Chat organisms | `internal/ui/chat` | Message, ToolRow (compose `ui` atoms) |
...
- **Tokenized only:** colors are `var(--token)` — **no raw hex**. (Raw `rgba()`
  is allowed for scrims/shadows, matching `--parch-bevel`.)
...
5. **Verify:**
   ...
   - Check **responsiveness** (the sidebar collapses to an off-canvas drawer
     ≤920px; wide components fill, don't overflow).
