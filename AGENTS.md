...
`plans/` holds implementation plans and reviews; `docs/adr/` holds architecture decision records. Phases 1–8 (foundation) of the file-canonical migration (ADR-0001) are implemented as the `storage/vault-*.js`, `storage/frontmatter.js`, `storage/entity-codec.js`, `storage/content-hash.js`, `storage/canvas-validate.js`, `storage/life-indexer.js`, `storage/memory-index.js`, `storage/workspace-vault.js`, `storage/workspace-backup.js`, `storage/task-repository.js`, `storage/task-migration.js`, `storage/habit-repository.js`, and `storage/journal-event-repository.js` library modules, with Node test suites `storage/phase1.test.js` through `storage/phase8.test.js` (run via `node --test`). `storage/life-store.js` carries schema version 2 (the rebuildable index tables). The task, habit, journal, and calendar-event repositories are the file-canonical backend (canonical `.md` entities, standard file-node placements, preservation-first edits, append-only habit check-in logs, per-date journals, timezone-aware events, index rebuild); `storage/task-migration.js` converts legacy marker tasks into that shape. Wiring the app's UI/Today to these repositories and running the migration against a real profile are the remaining browser-verified steps.
