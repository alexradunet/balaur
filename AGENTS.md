...
It boots a fresh profile and asserts: no uncaught console errors or failed assets; every document node renders; the sidebar reports the file-index status (canonical v1 loads **no SQLite**); the canvas tree’s chevron collapses a branch and persists the state to localStorage; clicking a card selects it and shows the corner-bracket selection frame (no circles); the sidebar stays visible and resizable while the inspector is open (note editing docks the inspector as a third column instead of swapping the sidebar); double-clicking *inside* a card and the note tool *on* a card create nothing; double-clicking empty background still creates a note; the top-bar color filter dims non-matching cards; the live document stays valid JSON Canvas 1.0; a controlled reload preserves the workspace; a collapsed tree branch survives the reload; and `--offline` confirms the Service Worker renders the shell from cache. Use `--profile <dir>` to reuse a profile across runs (first-run vs existing-profile / migration testing), `--width`/`--height` for narrow-shell checks, and `--screenshot <dir>` to dump a PNG for visual review. `eval` and `shot` subcommands run one-off runtime probes and screenshots. Read `.pi/skills/browser-check/SKILL.md` for recipes and the headless event-retargeting caveat. The skill is local agent tooling under `.pi/skills/` (tracked in the repo but not part of the deployed application shell); if it is absent (e.g. a fresh clone without `.pi/`), use the manual baseline list below instead.
